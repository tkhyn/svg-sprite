...
SVGObj.prototype.toSVG = function(inline) {
	...
	if (inline) {
		// remove everything not in the 1st found svg element
		var lcsvg = svg.toLowerCase()
		var svg_start = lcsvg.indexOf('<svg');
		var svg_end = lcsvg.lastIndexOf('</svg>');
		if (svg_start < 0 || svg_end < 0) {
			svg = '';
		} else {
			svg = svg.slice(svg_start, svg_end + 6);
		}
		...
		// remove XML declaration and doctype if applicable and if we're not in inline mode
		if (this._config._removeXMLProcInst) {
			svg = svg.replace(/<\?xml.*?>/, '');
		}
		if (this._config._removeDoctype) {
 ...
 */
SVGObj.prototype.getXMLProcInst = function() {
    var svg = this.svg.toString(false);
    var m = svg.match(/<\?xml.*?>/);
    return (m ? m[0] : '');
}
 ...
 */
SVGObj.prototype.getDoctype = function() {
    var svg = this.svg.toString(false);
    var m = svg.match(/<!DOCTYPE.*?>/);
    return (m ? m[0] : '');
}
