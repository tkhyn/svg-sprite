...
SVGSprite.prototype._addToSprite = function(svgID, svgInfo, svgPseudo, isLastSVG) {
	...
	// Save XML processing instructions if applicable
	if (this.XMLProcInst === undefined && this._options.cleanwith == 'svgo') {
		// we skip this step if this.XMLProcInst is already defineds
		var cc_plugins = this._options.cleanconfig.plugins; 
		for (var i = 0; i < cc_plugins.length; i++) {
			if (cc_plugins[i].removeXMLProcInst !== undefined) {
				if (cc_plugins[i].removeXMLProcInst === false) {
					this.XMLProcInst = true;
				}
				break;
			}
		}
		if (!this.XMLProcInst) {
			// the removeXMLProcInst render option is not defined, we set
			// XMLProcInst to false, so that we don't do the lookup for the
			// next SVG files
			this.XMLProcInst = false;
		}
	}
	if (this.XMLProcInst === true) {
		// now extract processing instruction from the current svg file if not
		// already extracted from a previous file
		var xml_proc_inst = svgInfo.getXMLProcInst();
		if (xml_proc_inst) {
			this.XMLProcInst = xml_proc_inst;
		}
	}
}
 ...
 * 
 * @param {Boolean}	inline	Prepare for inline usage (add XML namespaces if false)
 * @return {SVGSprite}		Self reference
 */
SVGSprite.prototype.toSVG = function(inline) {
	var svg					= [];
	if (inline === undefined) { inline = true; }
	if (!inline && typeof this.XMLProcInst == 'string') {
		// Insert XML processing instruction
		svg.push(this.XMLProcInst + '\n');
	}
