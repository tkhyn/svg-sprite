...
//		{transformsWithOnePath			: true}, // apply transforms, crop by real width, center vertical alignment and resize SVG with one Path inside
	...
	svgoNamesToCheck	= ['XMLProcInst', 'Doctype'];
...
function SVGSprite(inputDir, outputDir, options) {
	...
	// Check if SVGO's removeXMLProcInst and removeDoctype are disabled
	// assign values to relevant properties of this._options
	if (this._options.cleanwith == 'svgo') {
		var that = this;
		svgoNamesToCheck.forEach(function(name) {
			that._options['_remove' + name] = true;
		});
		var names = svgoNamesToCheck.concat();
		var cc_plugins = this._options.cleanconfig.plugins; 
		for (var i = 0; i < cc_plugins.length; i++) {
			names.concat().forEach(function (name, index) {
				if (cc_plugins[i]['remove' + name] === false) {
					that._options['_remove' + name] = false
...
//									.shortenIDs()
		...
		if (this._options.verbose > 2) {
			try {
				...
				size				= svgObj.toSVG(false).length,
...
SVGSprite.prototype._addToSprite = function(svgID, svgInfo, svgPseudo, isLastSVG) {
	...
	if (!('_svgoNamesToCheck' in this)) {
		this._svgoNamesToCheck = svgoNamesToCheck.concat();
	}
	var that = this;
	this._svgoNamesToCheck.concat().forEach(function(name, index) {
		if (that._options['_remove' + name]) {
			that._svgoNamesToCheck.splice(index, 1);
		} else if (that._options['_remove' + name] === false && !(('_' + name) in that)) {
			// Save XML block or Doctype from current file if applicable
			// For the sake of simplicity we use the values from the 1st SVG file where a value is defined
			var val = svgInfo['get' + name]();
			if (val) {
				that['_' + name] = val;
				that._svgoNamesToCheck.splice(index, 1);
			}
	...
	if (isLastSVG) {
		// cleanup on last file
		delete this._svgoNamesToCheck;
	}
}
